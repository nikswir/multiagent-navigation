...
%   d = json.load(open("report/assets/TD3_simpleEnv.json"))   # 174 epochs
%   w = d[-20:]                                               # 8-robot window
%   st.mean(e["Avg_arrived"]   for e in w)  -> 0.6737
%   st.mean(e["Avg_collision"] for e in w)  -> 0.3172
%   st.mean(e["Avg_timeout"]   for e in w)  -> 0.0091
%   st.mean(e["Avg_reward"]    for e in w)  -> -8.97
%   st.mean(e["Avg_reward"]    for e in d[:10]) -> -251.8
%   max(d, key=lambda e: e["Avg_arrived"])  -> epoch 46, 3 robots,
%                                              arrived 0.8367, collision 0.0667
\newcommand{\NumEpochs}{174}
\newcommand{\CurriculumCap}{8}
\newcommand{\LateArrived}{67.4\%}
\newcommand{\LateCollision}{31.7\%}
\newcommand{\LateTimeout}{0.9\%}
\newcommand{\LateReturn}{$-$9.0}
\newcommand{\EarlyReturn}{$-$252}
\newcommand{\BestArrived}{83.7\%}
\newcommand{\BestCollision}{6.7\%}
\newcommand{\BestEpoch}{46}
\newcommand{\BestRobots}{3}
...
a training curriculum. Over the final training window the deterministic
shared policy brings \LateArrived{} of robots to their goals per episode with
\CurriculumCap{} robots active; at \BestRobots{} robots the arrival rate
peaks at \BestArrived{}, isolating inter-robot interference as the dominant
failure mode.
\end{abstract}
...
uniformly from $\{1, \dots, n_{\max}(t)\}$, where the cap $n_{\max}(t)$ grows
linearly from $1$ to \CurriculumCap{} over the first $8\times10^5$ of the
$10^6$ training timesteps. Early training thus learns single-robot
...
shaded. The mean per-robot return climbs from \EarlyReturn{} in the first
ten epochs to around zero once single-robot navigation is mastered, and the
arrival rate exceeds $80\%$ while $2$--$3$ robots are active — peaking at
\BestArrived{} (collision rate \BestCollision) in epoch~\BestEpoch{} with
\BestRobots{} robots. As the curriculum ramps toward \CurriculumCap{}
robots, arrivals settle to \LateArrived{} and collisions grow to
\LateCollision{} (mean over the final $20$ epochs; timeouts almost vanish
at \LateTimeout), while the mean return stays near \LateReturn. The
degradation tracks the robot count, not training time: navigation itself is
learned early, and the remaining failures are dominated by robot--robot
interference — mutual blocking and collisions between robots that each
behave sensibly in isolation.
...
The same policy that delivers \BestArrived{} of robots at
$n=\BestRobots$ delivers \LateArrived{} at $n=\CurriculumCap$, with the
difference absorbed almost entirely by the collision rate
(\BestCollision{} $\to$ \LateCollision). Since finished robots freeze in
place and remain lidar-visible, late-episode worlds are also more cluttered
than the obstacle course alone. This isolates coordination — not
single-robot skill — as the binding constraint, which parameter sharing
alone does not resolve: the robots share competence but not intent.

% The success/failure trajectory pair is produced by the documented pipeline
% (run_experiment.py -> eval_policy.py -> make_figures.py); uncomment once
% report/assets/trajectory*.json exist and the figure is regenerated:
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.98\linewidth]{trajectory.pdf}
%   \caption{The deterministic shared policy on unseen episodes. Left: every
%   robot reaches its goal. Right: a characteristic failure — two paths cross
...
goals, lidar fans, trajectories) back every figure in this report and an
animated GIF of policy rollouts embedded in the repository README, so the
paper and the live environment are guaranteed to agree. All figures are
emitted as scalable PDF.
...
On real training data the shared policy masters single-robot navigation
early (over $80\%$ arrivals at $2$--$3$ robots) and retains \LateArrived{}
per-robot arrivals at the full \CurriculumCap-robot task, with robot--robot
collisions as the dominant remaining failure mode. Natural next steps target
exactly that gap: centralized critics with decentralized actors,
inter-robot communication or attention over neighbouring robots, explicit
collision-avoidance shaping between robots, prioritised replay, and
randomising the obstacle course itself for cross-layout generalization.
