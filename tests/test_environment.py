...
spawns, per-robot step semantics with the right terminal rewards, the shaped
step-reward formula, pose integration with heading wrap, the collision /
ray-cast / lidar primitives, out-of-bounds ending as a wall collision, and
the replay buffer's FIFO cap. `SimpleEnv` has no seed parameter, so each test
...
def _make_env(seed: int, n_robots: int, max_steps: int = 50) -> SimpleEnv:
    """A small seeded env: training-sized robots, capacity 4."""
    random.seed(seed)
    np.random.seed(seed)
    return SimpleEnv(
        robot_radius=0.25,
...
def test_step_returns_lists_per_robot_and_increments() -> None:
    ...
    states, rewards, dones, infos = env.step(np.zeros((3, 2)))
...
def test_target_arrival_terminates_with_bonus() -> None:
    ...
    # ── Park the robot on its goal; a zero action keeps it there ──
    env.robots[0].x = float(env.goals_x[0])
    env.robots[0].y = float(env.goals_y[0])
    _states, rewards, dones, infos = env.step(np.zeros((1, 2)))
...
def test_max_steps_terminates_alive_robots() -> None:
    ...
    env.goals_x[0] = 4.0
    env.goals_y[0] = 4.0
    _states, _rewards, dones, infos = env.step(np.zeros((1, 2)))
...
def test_reward_is_shaping_off_a_terminal() -> None:
    ...
    assert rewards[0] == pytest.approx(0.6 / 2 - 0.2 / 2 - 0.05)
...
def test_replay_buffer_fifo_cap_and_clear() -> None:
    ...
    # ── Capped at 3, oldest evicted first ──
    assert buffer.size() == 3
    assert [exp[2] for exp in buffer.buffer] == [2.0, 3.0, 4.0]
